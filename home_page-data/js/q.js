...
function openAllDialogTypes(){
	...
	function realTime(){
		...
		if(time = 1){
			if($("[data-show='dialog']").is(":visible") || $("[data-show='mobile-menu']").is(":visible") || $("[data-show='side-bar']").is(":visible") || $(".marketing-popup-overlay").is(":visible") ){
				$("body").addClass("open-customs");
			} else {
				$("body").removeClass("open-customs");
			}
...
function passwordSeeable(){
	$('.show-the-password').click(function(){
		$(this).hide(); 
		$(this).parent().find('input').attr('type', 'text'); 
		$(this).parent().find('.hide-the-password').fadeIn(); 
	});

	$('.hide-the-password').click(function(){
		$(this).hide(); 
		$(this).parent().find('input').attr('type', 'password'); 
		$(this).parent().find('.show-the-password').fadeIn(); 
	});
}
